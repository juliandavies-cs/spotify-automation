...
class MainPage:
    ...
    def search_for_song(self, test_case, song):
        ...
        search_bar = test_case.selenium_service.wait_until_located(self.search_bar_locator)
        search_bar.send_keys(f"{song['title']} - {song['artist']}")
        search_bar.send_keys(Keys.ENTER)
    
    def add_song(self, test_case, song):
        self.search_for_song(test_case, song)
        test_case.selenium_service.wait_until_element_text_to_be(self.first_song_title_locator, song['title'])
        test_case.selenium_service.wait_until_element_text_to_be(self.first_song_artist_locator, song['artist'])
        test_case.selenium_service.find_element(self.first_song_options_locator).click()
        test_case.selenium_service.hover_over_element(self.add_to_playlist_locator)
        test_case.selenium_service.wait_until_located(self.add_to_playlist_btn_locator).click()
