...
class PlaylistPage:
    ...
    def rename_playlist(self, playlist_name):
        playlist_name_btn = self.selenium_service.wait_until_located(self.playlist_name_btn_locator)
        playlist_name_btn.click()
        playlist_name_input = self.selenium_service.wait_until_located(self.edit_playlist_name_input_locator)
        playlist_name_input.send_keys(playlist_name)
        self.selenium_service.find_element(self.edit_playlist_save_btn_locator).click()

    def get_song_locator_based_on_count(self, count):
        return f"//div[@aria-rowindex='{count+2}']" 
    
    def delete_playlist(self):
        self.selenium_service.wait_until_located(self.more_button_locator).click()
        self.selenium_service.wait_until_located(self.delete_button_locator).click()
    ...
    def verify_songs_added(self, playlist_songs):
        try:
            for idx, song in enumerate(playlist_songs):
                ...
                self.selenium_service.wait_until_located(f"{song_locator}/div/div[2]/div/a/div")
                assert self.selenium_service.find_element(f"{song_locator}/div/div[2]/div/a/div").text == song['title']
                assert self.selenium_service.find_element(f"{song_locator}/div/div[2]/div/span/a").text == song['artist']
